...
## Sample Configurations and Results


**Configuration1: BatchLimit-10, Period2s**
...
Number of DataIngestPull commands: 27137 \
MaxSize: 0.4267578125 

**Configuration2: BatchLimit-1000, Period10s  (Default Config)**
...
Number of DataIngestPull commands: 1239\
MaxSize: 8

**Configuration3: BatchLimit-100000, Period60s**
...
Number of DataIngestPull commands: 72\
MaxSize: 307

## Example Queries:

**Query for DataIngestPull Count:**
  ```kql
  .show commands-and-queries 
  | where Database == "<DatabaseName>"  
  | where CommandType == "DataIngestPull" 
  | where Text has ("<TableName>") 
  | count  
```
...
**Query for MaxSize:**
  ```kql
KustoLogs
| where Source == "<ClsuterName>"
| where Timestamp > ago(30m)
| where EventText has "DownloadManager3.DownloadMany: Download complete."
| parse EventText with * 'maxSize='MaxSize', avgSize=' *
| extend MaxSize = toint(MaxSize)/1024.0
| summarize max(MaxSize)
```
